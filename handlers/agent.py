...
def _current_date() -> str:
    ...
    greg = f"{my_time.strftime('%A, %B %d, %Y %H:%M')}"
    try:
...
def _main_prompt() -> str:
    return (
        ...
        "IMPORTANT: Messages are wrapped with <sender>username</sender> to show who sent them. This is ONLY metadata for attribution. "
        "NEVER include the sender username in any search query, web_search, search_twitter, or tool call. "
        "Example: if '<sender>Daos Axia 1.0</sender> tempe ada kat speedmart?' — search for 'tempe speedmart', NOT 'Daos Axia 1.0 tempe speedmart'. "
        "The username is NOT part of the question. Extract only the actual query from the message text. "
        "Pronouns like 'it', 'that', 'them' refer to conversation context, NOT the sender. "
